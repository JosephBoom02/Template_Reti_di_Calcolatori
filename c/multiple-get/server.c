...
/********************************************************/
typedef struct {
    char targa[LENGHT_TARGA];
    char patente[LENGHT_PATENTE];
} udp_req_t;

typedef struct {
    char targa[LENGHT_TARGA];
    char patente[LENGHT_PATENTE];
    char tipo[LENGHT_TIPO];
    char folder_immagini[LENGHT_FOLDER_IMMAGINI];
} tabella;
/********************************************************/
...
int main(int argc, char **argv) {
    ...
    strcpy(stato[0].targa, "AN745NL");
    strcpy(stato[0].patente, "00003");
    strcpy(stato[0].tipo, "auto");
    strcpy(stato[0].folder_immagini, "AN745NL_img");
    for(int i=1;i<N;i++){
        strcpy(stato[i].targa, "L");
        strcpy(stato[i].patente, "0");
        strcpy(stato[i].tipo, "L");
        strcpy(stato[i].folder_immagini, "L");
    }
    ...
    for (;;) {
        ...
        if (FD_ISSET(udpfd, &rset)) {
            printf("Server: ricevuta richiesta di aggiornamento dello stato\n");
            res = -1;

            //stampo lo stato attuale
            printf("\nStato attuale:\n");
            for(int i=0; i<N; i++){
                printf("\tTarga: %s\tPatente: %s\tTipo:%s\n", stato[i].targa, stato[i].patente, stato[i].tipo);
            }

            //leggo la richiesta
            len = sizeof(struct sockaddr_in);
            if (recvfrom(udpfd, &req, sizeof(req), 0, (struct sockaddr *)&cliaddr, &len) <
                0) {
                perror("recvfrom");
                continue;
            }
            ...
            //aggiorno lo stato
            for (int i = 0; i<N ; i++) {
                if(strcmp(stato[i].targa, targa) == 0){
                    strcpy(stato[i].patente, patente);
                    res = 0;
                }
            }
            ...
            //mando il risultato dell'operazione
